"""Two more neighborhood recommenders: Slope One and co-clustering CF.

Both take ``triples`` -- ``(user, item, rating)`` -- like the rest of
``recommend``, and predict a held-out rating.
"""
import numpy as np
...
__all__ = ["SlopeOne", "CoClusteringCF"]
